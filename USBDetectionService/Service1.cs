...
namespace repeat
{
    public partial class Service4 : ServiceBase
    {
        ...
        public Service4()
        {
            ...
            if (!System.Diagnostics.EventLog.SourceExists("USBDetectionService"))
            {
                System.Diagnostics.EventLog.CreateEventSource(
                    "USBDetectionService", "USBDetectionService");
            }
            //OnStart(null);
        }
        ...
        public void USBInserted(object sender, EventArgs e)
        {
            ...
            using (StreamWriter wr = new StreamWriter(@"E:\UsbDetected1.txt", false))
            {
            ...
            try
            {
                ...
                using (channel as IDisposable)
                {
                    try
                    {
                        channel.PrintVirusDetails(new FilePath { File_Path = "USB", Virus_Type = "Detected Scanning in progress..." });
                    }
                    catch (Exception ex)
                    {
        ...
        void USBRemoved(object sender, EventArgs e)
        {
            ...
            using (StreamWriter wr = new StreamWriter(@"E:\UsbDetected.txt", false))
            {
            ...
            //        try
            //        {
            //            channel.PrintVirusDetails(new FilePath { File_Path = "USB", Virus_Type = "Removed" });
            //        }
            //        catch (Exception ex)
            //        {
        ...
        public void clamscan(string path)
        {
            ...
            Task.Run(() =>
            {
                var clam = new ClamClient("localhost", 3310);
                var t = path.Replace(@"\", "");
                //        var scanResult = await clam.ScanFileOnServerAsync(path);  //any file you would like!
                ...
                switch (scanResult.Result)
                {
                    case ClamScanResults.Clean:
                        ...
                        using (StreamWriter wr = new StreamWriter(@"E:\UsbDetectedResult.txt", false))
                        {
                        ...
                        //        try
                        //        {
                        //            channel.PrintVirusDetails(new FilePath { File_Path = path, Virus_Type = scanResult.RawResult.Split(':')[2].Split(' ')[1] });
                        //        }
                        //        catch (Exception ex)
                        //        {
