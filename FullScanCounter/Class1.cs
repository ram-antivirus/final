...
namespace FullScanCounter
{
    public static class Class1
    {
       
        public static int Count(string name,BlockingCollection<string> bc)
        {
       ...
       //     List<String> mylist = new List<String>();
            ...
            try
            {
                ...
                StringBuilder fullPath = new StringBuilder(" "); //A deep search of the Internet enlighted me of the advantage of using StringBuilder over String
                 //The final list that will have all the paths
                ...
                foreach (var entry in mDict)
                {
                    var sample = entry;
                    fullPath = new StringBuilder(sample.Value.Name);
                    FileNameAndParentFrn keysWithMatchingValues;
                    mft.Directories.TryGetValue(sample.Value.ParentFrn, out keysWithMatchingValues);

                    while (true)
                    {
                        if (keysWithMatchingValues == null)
                            break;
                        fullPath.Insert(0, keysWithMatchingValues.Name + "\\");
                        sample.Value.ParentFrn = keysWithMatchingValues.ParentFrn;
                        mft.Directories.TryGetValue(sample.Value.ParentFrn, out keysWithMatchingValues);
                    }
                    pathList.Add(fullPath.Replace(@"\\.\", "").ToString());
                }

                foreach (var a in Partition(pathList, pathList.Count / 10))
                {
                    //if (File.Exists(name.TrimEnd(':') + ".bin"))
                    //{
                    //    File.Delete(a + ".bin");
                    //    Console.WriteLine("deleted" + a + ".bin");
                    //}
                    //else
                    //    Console.WriteLine("not deleted");
                    using (FileStream stream = new FileStream(AppDomain.CurrentDomain.BaseDirectory + name.TrimEnd(':') + ".bin", FileMode.Append))
                    {
                        BinaryFormatter bin = new BinaryFormatter();
                        bin.Serialize(stream, a);
                    }
                }
         //       FullScanCounter.Properties.Settings.Default.myList1.Add(name);
                bc.Add(name);
                Console.WriteLine("Done with>>>>>>"+name );
                finalcount = pathList.Count;
                pathList.Clear();
                pathList = null;
                mDict.Clear();
                mDict = null;
...
{
    public class PInvokeWin32 : IDisposable
    {
        ...
        private void GetRootFrnEntry()
        {
            ...
            if (hRoot.ToInt32() != PInvokeWin32.INVALID_HANDLE_VALUE)
            {
                ...
                if (bRtn)
                {
                    ...
                    FileNameAndParentFrn f = new FileNameAndParentFrn(driveRoot, 0, 0);
                    ...
                    if (0 != (usn.FileAttributes & PInvokeWin32.FILE_ATTRIBUTE_DIRECTORY))
                    {
                        ...
                        if (!_directories.ContainsKey(usn.FileReferenceNumber))
                        {
                            _directories.Add(usn.FileReferenceNumber,
                                new FileNameAndParentFrn(usn.FileName, usn.ParentFileReferenceNumber, usn.FileReferenceNumber));
                        }
                        else
